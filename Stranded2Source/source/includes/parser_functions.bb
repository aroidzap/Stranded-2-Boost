...
Function loadstring_parsecache(txt$,sep$="¦",clear=1)
...
Function set_parsecache(class,id,event$="",info$="")
		...
		If Tx\mode=0 Then
			If Tx\parent_class=class Then
				If Tx\parent_id=id Then
					;Event Check
					If event$<>"" Then
						If Instr(Tx\key$,"¦"+event$)=0 Then Return 0
					EndIf
					;Add Parse Task
					Tpt.Tpt=New Tpt
					Tpt\class=class
	...
	If class=0 Then
		If id=0 Then
			If Instr(map_briefing_key$,"¦"+event$) Then
				;Add Parse Task
				Tpt.Tpt=New Tpt
				Tpt\class=0
				Tpt\id=0
...
Function parse_globalevent(event$,info$="")
	;Game Script
	If Instr(game_scriptk$,"¦"+event$) Then
		;Add Parse Task
		Tpt.Tpt=New Tpt
		Tpt\class=-1
		Tpt\id=0
	...
	For i=1 To Cobject_count
		If Instr(Dobject_scriptk$(i),"¦"+event$) Then
	...
	For i=1 To Cunit_count
		If Instr(Dunit_scriptk$(i),"¦"+event$) Then
	...
	For i=1 To Citem_count
		If Instr(Ditem_scriptk$(i),"¦"+event$) Then
	...
	For i=1 To Cinfo_count
		If Instr(Dinfo_scriptk$(i),"¦"+event$) Then
...
Function parse_all()
	...
	For Tpt.Tpt=Each Tpt
		...
		p_env_info$=Tpt\info$
		loadstring_parsecache(Tpt\script$,"¦")
		parse()
		Delete Tpt
	Next
	...
	Select class
		Case Cclass_object
			If con_object(id) Then
				If Instr(Dobject_scriptk$(TCobject\typ),"¦"+event$) Then
					parse_task(class,id,event$,info$,Dobject_script(TCobject\typ))
				EndIf
			EndIf
		Case Cclass_unit
			If con_unit(id) Then
				If Instr(Dunit_scriptk$(TCunit\typ),"¦"+event$) Then
					parse_task(class,id,event$,info$,Dunit_script(TCunit\typ))
				EndIf
			EndIf
		Case Cclass_item
			If con_item(id) Then
				If Instr(Ditem_scriptk$(TCitem\typ),"¦"+event$) Then
					parse_task(class,id,event$,info$,Ditem_script(TCitem\typ))
				EndIf
			EndIf
		Case Cclass_info
			If con_info(id) Then
				If Instr(Dinfo_scriptk$(TCinfo\typ),"¦"+event$) Then
...
Function parse_editorsetscript(class,id)
	...
	If ed_setscript$<>"" Then
		...
		loadstring_parsecache(ed_setscript$,"¦")
		parse(1)
	EndIf
	;Edset-Event
